...
const DOM_ELEMENTS = {
    ...
    nextLessonTime: null,
    earlyMorningCard: null,
    earlyLessonTitle: null,
    earlyLessonTime: null,
    afterClassesCard: null,
    // Dashboard View
    dashTimer: null,
    dashNum: null,
    dashRange: null,
...
function getCurrentState(currentTime) {
    ...
    // === ПЕРЕВІРКА ЧАСУ ДО 07:00 ===
    const dayStartTime = 7 * 60; // 07:00 в хвилинах
    if (currentTime < dayStartTime) {
        return {
            type: 'before_day_start',
            lesson: BELLS_SCHEDULE[0],
            nextEventTime: timeToMinutes(BELLS_SCHEDULE[0].start),
            statusText: 'Навчальний день ще не почався'
        };
    }
    // ================================
...
function renderEarlyMorningCard(nextLesson, currentTime) {
    if (!DOM_ELEMENTS.earlyMorningCard) return;

    if (DOM_ELEMENTS.earlyLessonTitle) DOM_ELEMENTS.earlyLessonTitle.textContent = `${nextLesson.id} пара`;
    if (DOM_ELEMENTS.earlyLessonTime) DOM_ELEMENTS.earlyLessonTime.textContent = `${nextLesson.start} - ${nextLesson.end}`;

    const dayStartTime = 7 * 60; // 07:00

    if (currentTime < dayStartTime) {
        // === ДО 07:00: НЕ РАХУЄМО ===
        if (DOM_ELEMENTS.dashLabel) {
            DOM_ELEMENTS.dashLabel.textContent = "РАНО";
            DOM_ELEMENTS.dashLabel.className = "text-[10px] font-bold uppercase tracking-widest";
            DOM_ELEMENTS.dashLabel.style.color = '#6366f1'; // indigo
        }

        if (DOM_ELEMENTS.dashTimer) DOM_ELEMENTS.dashTimer.textContent = "--";
        if (DOM_ELEMENTS.dashDesc) DOM_ELEMENTS.dashDesc.textContent = "Ще рано...";
    } else {
        // === ПІСЛЯ 07:00: РАХУЄМО ===
        if (DOM_ELEMENTS.dashLabel) {
            DOM_ELEMENTS.dashLabel.textContent = "ОЧІКУВАННЯ";
            DOM_ELEMENTS.dashLabel.className = "text-[10px] font-bold uppercase tracking-widest";
            DOM_ELEMENTS.dashLabel.style.color = '#0ea5e9'; // sky blue
        }
    ...
    if (DOM_ELEMENTS.dashRange) DOM_ELEMENTS.dashRange.textContent = `Перша: ${nextLesson.start}`;

    // Змінюємо іконку залежно від часу теж (опціонально)
    if (DOM_ELEMENTS.dashNum) {
        if (currentTime < dayStartTime) {
            DOM_ELEMENTS.dashNum.innerHTML = '<i class="fa-solid fa-moon text-xl"></i>';
        } else {
            DOM_ELEMENTS.dashNum.innerHTML = '<i class="fa-solid fa-sun text-xl"></i>';
        }
    }
}

function renderAfterClassesCard() {
    // Дашборд після пар
    if (DOM_ELEMENTS.dashLabel) {
        DOM_ELEMENTS.dashLabel.textContent = "ВІЛЬНИЙ";
        DOM_ELEMENTS.dashLabel.className = "text-[10px] font-bold uppercase tracking-widest";
        DOM_ELEMENTS.dashLabel.style.color = '#10b981'; // emerald
    }

    if (DOM_ELEMENTS.dashTimer) DOM_ELEMENTS.dashTimer.textContent = "--";
    if (DOM_ELEMENTS.dashDesc) DOM_ELEMENTS.dashDesc.textContent = "Пари закінчилися";
    if (DOM_ELEMENTS.dashRange) DOM_ELEMENTS.dashRange.textContent = "Відпочивай";

    if (DOM_ELEMENTS.dashNum) DOM_ELEMENTS.dashNum.innerHTML = '<i class="fa-solid fa-circle-check text-xl"></i>';
}
...
function updateTimeAndSchedule() {
    ...
    if (state) {
        ...
        if (state.type !== previousState.type) {
            ...
            if (DOM_ELEMENTS.earlyMorningCard) DOM_ELEMENTS.earlyMorningCard.style.display = 'none';
            if (DOM_ELEMENTS.afterClassesCard) DOM_ELEMENTS.afterClassesCard.style.display = 'none';
            ...
            if (state.type === 'weekend') {
            ...
            else if (state.type === 'before_day_start') {
                // === РАНО ВРАНЦІ (до 07:00) ===
                if (DOM_ELEMENTS.earlyMorningCard) DOM_ELEMENTS.earlyMorningCard.style.display = 'flex';
                renderEarlyMorningCard(state.lesson, currentTime);
            }
            else if (state.type === 'big_break') {
                // === ВЕЛИКА ПЕРЕРВА (між парами) ===
                ...
                // === ДО ПОЧАТКУ ПАР (07:00 - 08:30) ===
                if (DOM_ELEMENTS.earlyMorningCard) DOM_ELEMENTS.earlyMorningCard.style.display = 'flex';
                renderEarlyMorningCard(state.lesson, currentTime);
            }
            else if (state.type === 'after_classes') {
                // === ПІСЛЯ ПАР ===
                if (DOM_ELEMENTS.afterClassesCard) DOM_ELEMENTS.afterClassesCard.style.display = 'flex';
                renderAfterClassesCard();
            }
            ...
            } else if (state.type === 'before_day_start' || state.type === 'before_classes') {
                updateBigBreakTimer(state.lesson, currentTime);
            }
        }
...
function updateBigBreakTimer(nextLesson, currentTime) {
    ...
    if (currentTime < dayStartTime) {
        // === ДО 07:00: НЕ РАХУЄМО ===
        if (DOM_ELEMENTS.dashTimer) {
            DOM_ELEMENTS.dashTimer.textContent = "--";
        }
        return;
    }
...
function initDOMElements() {
    ...
    if (notificationsView || dashboardView) {
        ...
        DOM_ELEMENTS.earlyMorningCard = document.getElementById('early-morning-card');
        DOM_ELEMENTS.earlyLessonTitle = document.getElementById('early-lesson-title');
        DOM_ELEMENTS.earlyLessonTime = document.getElementById('early-lesson-time');

        DOM_ELEMENTS.afterClassesCard = document.getElementById('after-classes-card');
